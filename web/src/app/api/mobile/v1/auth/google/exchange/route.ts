import { MobileAuthExchangeRequestSchema } from '@forja/domain'
...
export async function POST(request: Request): Promise<Response> {
  ...
  const payload = MobileAuthExchangeRequestSchema.safeParse(
  ...
  try {
    const result = await createDefaultMobileAuthService().exchangeMobileAuthCode(
      payload.data,
    )
